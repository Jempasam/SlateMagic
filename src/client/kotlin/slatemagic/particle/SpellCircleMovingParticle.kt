...
import slatemagic.shape.painter.IncompletePartPainter
import slatemagic.shape.painter.vertex.ParticleVPC
import kotlin.math.abs
import kotlin.math.sin

class SpellCircleMovingParticle(
    ...
    override fun buildGeometry(vertexConsumer: VertexConsumer, camera: Camera, tickDelta: Float) {
        ...
        val vibration = (sin(time/4f)/20+0.1f)/size
        ...
        run {
            val painter = IncompletePartPainter(
                CircleVertexPainter(ParticleVPC(sprite, matrix, vertexConsumer), color, 0.1f/size, vibration),
                updown / 2.5
            )
        ...
        run{
            val painter=IncompletePartPainter(
                CircleVertexPainter(ParticleVPC(sprite, matrix, vertexConsumer), 0xffffff, 0.05f/size, vibration),
                updown / 3.5
            )
            shape.draw(painter, time/10.0/size)
        }
    ...
    class Factory(val sprite: SpriteProvider): ParticleFactory<SpellCircleParticleEffect>{
        override fun createParticle(effect: SpellCircleParticleEffect, world: ClientWorld, x: Double, y: Double, z: Double, vx: Double, vy: Double, vz: Double): SpellCircleMovingParticle {
            return SpellCircleMovingParticle(sprite, world, x, y, z, vx, vy, vz, effect)
        }
    }
}