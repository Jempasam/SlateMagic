...
object SlabMagicBlocks {
    fun <T: Block> create(id: String, block: T): T{
        Registry.register(Registries.BLOCK, SlabMagicMod.id(id), block)
        return block
    }
    ...
    val COPPER_ROBOT= create("copper_robot", RobotBlock(AbstractBlock.Settings.copy(Blocks.COPPER_BLOCK), WandItem::cast, 1, 30.0, 40))
    val GOLD_ROBOT= create("gold_robot", RobotBlock(AbstractBlock.Settings.copy(Blocks.NETHERITE_BLOCK), WandItem::cast, 2, 60.0, 80))
    val ANCIENT_ROBOT= create("ancient_robot", RobotBlock(AbstractBlock.Settings.copy(Blocks.OXIDIZED_COPPER).strength(100F,300F), WandItem::cast, 4, 80.0, 120))
