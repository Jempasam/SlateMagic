...
class ExplosionSpellEffect(val power: Float=1f, val createFire: Boolean=false): SpellEffect {
    override fun use(context: SpellContext): SpellContext {
        context.world.createExplosion(null, context.pos.x, context.pos.y, context.pos.z, 1.0f+power*context.stored.power.toFloat()/2, createFire, World.ExplosionSourceType.MOB)
        val ppower=context.stored.power/10.0
        sendParticleEffect(
            context.world,
            ...
            context.stored.power*10.0*power
        )
        return context
    }
    ...
    override val color: Vector3f get() = ColorTools.vec(if(createFire) DyeColor.ORANGE.signColor else DyeColor.RED.signColor)
